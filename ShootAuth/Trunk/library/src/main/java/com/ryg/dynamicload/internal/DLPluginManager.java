...
import com.ryg.dynamicload.DLBasePluginApplication;
import com.ryg.dynamicload.DLBasePluginBroadcastReceiver;
import com.ryg.dynamicload.DLBasePluginContentProvider;
import com.ryg.dynamicload.DLBasePluginFragmentActivity;
import com.ryg.dynamicload.DLBasePluginService;
import com.ryg.dynamicload.DLBasePluginTranslucentFragmentActivity;
import com.ryg.dynamicload.DLProxyActivity;
import com.ryg.dynamicload.DLProxyApplication;
import com.ryg.dynamicload.DLProxyBroadcastReceiver;
import com.ryg.dynamicload.DLProxyContentProvider;
import com.ryg.dynamicload.DLProxyFragmentActivity;
import com.ryg.dynamicload.DLProxyService;
import com.ryg.dynamicload.DLProxyTranslucentFragmentActivity;
...
public class DLPluginManager {
    ...
    private DLPluginManager(Context context) {
        LOG.d(TAG, "DLPluginManager context="+context);
        mContext = context.getApplicationContext();
        LOG.d(TAG, "DLPluginManager mContext="+mContext);
    ...
    public void reloadApk(String dexPath) {
...
//            if (packageInfo.versionCode > pluginPackage.packageInfo.versionCode) {
//                mPackagesHolder.remove(packageInfo.packageName);
//                File file = new File(mContext.getDir("dex", Context.MODE_PRIVATE) + File.separator + packageInfo.packageName + ".dex");
//                if (file.exists()) {
//                    file.delete();
//                }
//            } else {
...
//            }
    ...
    private DexClassLoader createDexClassLoader(String dexPath) {
        ...
        LOG.d(TAG, "createDexClassLoader dexOutputPath="+dexOutputPath);
        DexClassLoader loader = new DexClassLoader(dexPath, dexOutputPath, mNativeLibDir, mContext.getClassLoader());
        LOG.d(TAG, "createDexClassLoader loader="+loader);
        return loader;
    }
    ...
    public int startPluginApplication(DLProxyApplication application, String packageName, String className) {
        ...
        Class<? extends Application> applicationClass = getApplicationClass(clazz);
        if (applicationClass == null) {
            return START_RESULT_TYPE_ERROR;
        }

        pluginPackage.isLoaded = true;
        application.onCreatePlugin(packageName, className);
        return START_RESULT_SUCCESS;
    }

    public int startPluginContentProvider(DLProxyContentProvider contentProvider, String packageName, String className) {
        ...
        Class<? extends ContentProvider> contentProviderClass = getContentProviderClass(clazz);
        if (contentProviderClass == null) {
            return START_RESULT_TYPE_ERROR;
        }

        contentProvider.onCreatePlugin(packageName, className);

        return START_RESULT_SUCCESS;
    }

    public int startPluginBroadcastReceiver(DLProxyBroadcastReceiver broadcastReceiver, Context context, String packageName, String className, String[] actions) {
        ...
        Class<? extends BroadcastReceiver> broadcastReceiverClass = getBroadcastReceiverClass(clazz);
        if (broadcastReceiverClass == null) {
            return START_RESULT_TYPE_ERROR;
        }

        broadcastReceiver.onCreatePlugin(context, packageName, className, actions);

        return START_RESULT_SUCCESS;
    }

    public int startPluginService(final Context context, final DLIntent dlIntent) {
        LOG.d(TAG, TAG + " startPluginService mFrom="+mFrom);
        if (mFrom == DLConstants.FROM_INTERNAL) {
            dlIntent.setClassName(context, dlIntent.getPluginClass());
        ...
        fetchProxyServiceClass(dlIntent, new OnFetchProxyServiceClass() {
            ...
            public void onFetch(int result, Class<? extends Service> proxyServiceClass) {
                ...
                if (result == START_RESULT_SUCCESS) {
                    LOG.d(TAG, "startPluginService proxyServiceClass="+proxyServiceClass);
    ...
    private void fetchProxyServiceClass(DLIntent dlIntent, OnFetchProxyServiceClass fetchProxyServiceClass) {
        String packageName = dlIntent.getPluginPackage();
        LOG.d(TAG, "fetchProxyServiceClass packageName="+packageName);
        if (TextUtils.isEmpty(packageName)) {
            throw new NullPointerException("disallow null packageName.");
        }
        DLPluginPackage pluginPackage = mPackagesHolder.get(packageName);
        ...
        LOG.d(TAG, "fetchProxyServiceClass className="+className);
        LOG.d(TAG, "fetchProxyServiceClass classLoader="+pluginPackage.classLoader);
        Class<?> clazz = loadPluginClass(pluginPackage.classLoader, className);
        if (clazz == null) {
    ...
    private Class<?> loadPluginClass(ClassLoader classLoader, String className) {
        ...
        try {
            ...
            LOG.d(TAG, "loadPluginClass clazz="+clazz);
        } catch (ClassNotFoundException e) {
            LOG.d(TAG, "loadPluginClass message="+e.toString());
            e.printStackTrace();
        }
    ...
    private Class<? extends Activity> getProxyActivityClass(Class<?> clazz) {
        ...
        } else if (DLBasePluginTranslucentFragmentActivity.class.isAssignableFrom(clazz)) {
            activityClass = DLProxyTranslucentFragmentActivity.class;
        }

        return activityClass;
    }

    private Class<? extends ContentProvider> getContentProviderClass(Class<?> clazz) {
        Class<? extends ContentProvider> proxyContentProviderClass = null;
        if (DLBasePluginContentProvider.class.isAssignableFrom(clazz)) {
            proxyContentProviderClass = DLProxyContentProvider.class;
        }
        return proxyContentProviderClass;
    }

    private Class<? extends BroadcastReceiver> getBroadcastReceiverClass(Class<?> clazz) {
        Class<? extends BroadcastReceiver> proxyBroadcastReceiverClass = null;
        if (DLBasePluginBroadcastReceiver.class.isAssignableFrom(clazz)) {
            proxyBroadcastReceiverClass = DLProxyBroadcastReceiver.class;
        }
        return proxyBroadcastReceiverClass;
    }

    private Class<? extends Application> getApplicationClass(Class<?> clazz) {
        Class<? extends Application> proxyApplicationClass = null;
        if (DLBasePluginApplication.class.isAssignableFrom(clazz)) {
            proxyApplicationClass = DLProxyApplication.class;
        }
        return proxyApplicationClass;
    }
    ...
    private interface OnFetchProxyApplicationClass {
        public void onFetch(int result, Class<? extends Application> proxyApplicationClass);
    }
    ...
    public boolean verifyPlugin(String pluginPath, PackageInfo packageInfo) {
        DLPluginPackage pluginPackage = getPackage(packageInfo.packageName);
        if (pluginPackage == null) {
        ...
        if (currentPluginVersionCode > loadedPluginVersionCode) {
