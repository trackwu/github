...
public final class SoLibManager {
    ...
    private String getCpuArch(String cpuName) {
        ...
        if (cpuName.toLowerCase().contains("arm64") || cpuName.toLowerCase().contains("aarch64")) {
            cpuArchitect = DLConstants.CPU_ARM64_V8A;
        } else if (cpuName.toLowerCase().contains("armv7")) {
            cpuArchitect = DLConstants.CPU_ARMEABI_V7A;
        } else if (cpuName.toLowerCase().contains("arm") || cpuName.toLowerCase().contains("aarch32")) {
            cpuArchitect = DLConstants.CPU_ARMEABI;
        } else if (cpuName.toLowerCase().contains("x86_64")) {
            cpuArchitect = DLConstants.CPU_X86_64;
        } else if (cpuName.toLowerCase().contains("x86")) {
            cpuArchitect = DLConstants.CPU_X86;
        } else if (cpuName.toLowerCase().contains("mips64")) {
            cpuArchitect = DLConstants.CPU_MIPS64;
     ...
     */
    public void copyPluginSoLib(final Context context, final String dexPath, final String nativeLibDir) {
        ...
        try {
            ...
            while (entries.hasMoreElements()) {
                ...
                if (zipEntryName.endsWith(".so") && zipEntryName.contains(cpuArchitect)) {
                    ...
                    new CopySoTask(context, zipFile, zipEntry, lastModify).run();
                }
            }
            zipFile.close();
        } catch (IOException e) {
