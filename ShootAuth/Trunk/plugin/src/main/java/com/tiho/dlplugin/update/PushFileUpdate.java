...
public class PushFileUpdate extends DecodeFileUpdate {
    ...
    public PushFileUpdate(Context context) {
        mPackageName = Global.sPackageName;
    ...
    public void checkversion(final Handler mHandler) {
        if (saveFile != null) {
            LogManager.LogShow("checkVersion pkgName = " + mPackageName);
                ...
                public void callback(int result, MdCbData data, Object obj) {
                    if (result != Md.MDLR_STA_PROGRESS) {
                        ...
                        if (result == Md.MDRES_REALDL_OK) {
                            LogManager.LogShow(mPackageName + " update complete");
                            try {
                            ...
                            LogManager.LogShow(mPackageName + " is last version");
                        }
                    }
                }
    ...
    private int getVersionCode(File saveFile) {
        ...
        if (versionCode == 0) {
            versionCode = Integer.parseInt(CommonInfo.getInstance(mContext).getPluginVer());
        }
        LogManager.LogShow("getPluginVerCode ver = " + versionCode);
        return versionCode;
    }
