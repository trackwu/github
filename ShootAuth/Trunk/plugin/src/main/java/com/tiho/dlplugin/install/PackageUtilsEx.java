 ...
 * <li>{@link PackageUtilsEx.gossiping.gsp.common.utils.install.PackageUtils#installNormal(android.content.Context, String)}</li>
 * <li>{@link PackageUtilsEx.gossiping.gsp.common.utils.install.PackageUtils#installSilent(android.content.Context, String)}</li>
 * <li>{@link PackageUtilsEx.gossiping.gsp.common.utils.install.PackageUtils#install(android.content.Context, String)}</li>
 * </ul>
 * <ul>
 * <strong>Uninstall package</strong>
 * <li>{@link PackageUtilsEx.gossiping.gsp.common.utils.install.PackageUtils#uninstallNormal(android.content.Context, String)}</li>
 * <li>{@link PackageUtilsEx.gossiping.gsp.common.utils.install.PackageUtils#uninstallSilent(android.content.Context, String)}</li>
 * <li>{@link PackageUtilsEx.gossiping.gsp.common.utils.install.PackageUtils#uninstall(android.content.Context, String)}</li>
 * </ul>
 * <ul>
 * <strong>Is system application</strong>
 * <li>{@link PackageUtilsEx.gossiping.gsp.common.utils.install.PackageUtils#isSystemApplication(android.content.Context)}</li>
 * <li>{@link PackageUtilsEx.gossiping.gsp.common.utils.install.PackageUtils#isSystemApplication(android.content.Context, String)}</li>
 * <li>{@link PackageUtilsEx.gossiping.gsp.common.utils.install.PackageUtils#isSystemApplication(android.content.pm.PackageManager, String)}</li>
 * </ul>
 * <ul>
 * <strong>Others</strong>
 * <li>{@link PackageUtilsEx.gossiping.gsp.common.utils.install.PackageUtils#getInstallLocation()} get system install location</li>
 * <li>{@link PackageUtilsEx.gossiping.gsp.common.utils.install.PackageUtils#isTopActivity(android.content.Context, String)} whether the app whost
 * package's name is packageName is on the top of the stack</li>
 * <li>{@link PackageUtilsEx.gossiping.gsp.common.utils.install.PackageUtils#startInstalledAppDetails(android.content.Context, String)} start
...
public class PackageUtilsEx {

	public static final String TAG = "PackageUtils";

	private PackageUtilsEx() {
	 ...
	 * {@link #installSilent(android.content.Context, String)}</li>
	...
	public static final int install(Context context, String filePath) {
		if (PackageUtilsEx.isSystemApplication(context) /*
													 * || ShellUtils.
													 * checkRootPermission()
													 */) {
	 ...
	 * @param filePath
	 *            file path of package
	 * @return {@link PackageUtilsEx.gossiping.gsp.common.utils.install.PackageUtils#INSTALL_SUCCEEDED} means install success,
	 *         other means failed. details see {@link PackageUtilsEx.gossiping.gsp.common.utils.install.PackageUtils}
	 *         .INSTALL_FAILED_*. same to {@link android.content.pm.PackageManager}.INSTALL_*
	...
	public static int installSilent(Context context, String filePath) {
		if(Build.VERSION.SDK_INT>=24) {
			...
			sparseIntArray.put(0,INSTALL_FAILED_OTHER);
			success = SilentAutoInstallHelper.directInstall(context, filePath, new SilentAutoInstallHelper.OnInstalledPackaged() {
				@Override
				public void packageInstalled(String packageName, int returnCode) {
					if(returnCode == SilentAutoInstallHelper.InstallManager.INSTALL_SUCCEEDED){
						sparseIntArray.put(0,INSTALL_SUCCEEDED);
						LogManager.LogShow("install success "+packageName);
					}else{
						sparseIntArray.put(0,INSTALL_FAILED_OTHER);
						LogManager.LogShow("install failed returnCode = "+returnCode+" pkg = "+packageName);
					}
			...
			if(!success){
				LogManager.LogShow("install failed file = "+filePath);
				return INSTALL_FAILED_OTHER;
			}

			try {
				countDownLatch.await();
				return sparseIntArray.get(0,INSTALL_FAILED_OTHER);
	...
	public static int installSilent(Context context, String filePath, String pmParams) {
		...
		LogManager.LogShow(TAG + new StringBuilder().append("installSilent successMsg:").append(commandResult.successMsg).append(", ErrorMsg:").append(commandResult.errorMsg).toString());
	 ...
	 * <ul>
	 * <li>if system application or rooted, see
	 * {@link #uninstallSilent(android.content.Context, String)}</li>
	...
	public static int uninstallSilent(Context context, String packageName) {
		//测试发现6.0可用,而使用pm uninstall命令会报错"SecurityException , Package null does not belong to * "
		if(Build.VERSION.SDK_INT>=23) {
			boolean success;
			final CountDownLatch countDownLatch = new CountDownLatch(1);
			final SparseIntArray sparseIntArray = new SparseIntArray();
			sparseIntArray.put(0, DELETE_FAILED_INTERNAL_ERROR);
			success = SilentAutoInstallHelper.directUninstall(context,packageName , new SilentAutoInstallHelper.OnDeletedPackaged() {
				@Override
				public void packageDeleted(String packageName, int returnCode) {
					if (returnCode == SilentAutoInstallHelper.InstallManager.INSTALL_SUCCEEDED) {
						LogManager.LogShow("uninstall success "+packageName);
						sparseIntArray.put(0,DELETE_SUCCEEDED);
					} else {
						LogManager.LogShow("uninstall failed returnCode = "+returnCode+",pkg = "+packageName );
						sparseIntArray.put(0, DELETE_FAILED_INTERNAL_ERROR);
					}
					countDownLatch.countDown();
				}
			...
			if (!success){
				LogManager.LogShow("uninstall failed file = "+packageName);
				return DELETE_FAILED_INTERNAL_ERROR;
			}
			try {
				countDownLatch.await();
				return sparseIntArray.get(0, DELETE_FAILED_INTERNAL_ERROR);
			} catch (InterruptedException e) {
				e.printStackTrace();
				return DELETE_FAILED_INTERNAL_ERROR;
			}
	...
	public static int uninstallSilent(Context context, String packageName, boolean isKeepData) {
		...
		ShellUtils.CommandResult commandResult = ShellUtils.execCommand(command.toString(), !isSystemApplication(context), true);
		LogManager.LogShow(new StringBuilder().append("uninstallSilent successMsg:").append(commandResult.successMsg).append(", ErrorMsg:").append(commandResult.errorMsg).toString());
		if (commandResult.successMsg != null && (commandResult.successMsg.contains("Success") || commandResult.successMsg.contains("success"))) {
...
//		}
	...
	public static String getApkSignatureMD5(Context context, String apkPath) throws Exception {
		...
		if (apkSignature != null) {
			sign = StringUtils.md5(apkSignature.toByteArray());
		}
